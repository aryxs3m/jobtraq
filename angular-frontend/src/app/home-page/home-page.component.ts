...
export class HomePageComponent implements OnInit {
  ...
  protected loadCharts() {
    ...
    this.http
      .get<HomePageReportResponse>(environment.api_url + apiRoute)
      .subscribe(
        data => {
          ...
          if (openPositionsWeeks > 1) {
            const distance =
              this.barOpenPositions[openPositionsWeeks - 2].value -
              this.barOpenPositions[openPositionsWeeks - 1].value;
            if (distance > 5) {
